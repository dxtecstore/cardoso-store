...
export default function CartDrawer() {
  ...
  const stepTitle = { cart: 'Carrinho', checkout: 'Finalizar Pedido', success: 'Pedido Enviado!' }
          ...
          {step === STEPS.CHECKOUT && (
            <div className="space-y-4 pt-1">
              ...
              <div>
                ...
                <input name="phone" value={form.phone} onChange={handleChange} className="dm-input" placeholder="(85) 99999-9999" inputMode="tel" />
              </div>
              <div>
              ...
              <div className="border-t border-dm-border pt-4 space-y-1">
                {items.map(i => (
                  <div key={i.key} className="flex justify-between text-[10px] text-dm-muted py-0.5">
                    <span>{i.product.title} ({i.size}) ×{i.qty}</span>
                    <span>R$ {(i.product.price * i.qty).toFixed(2)}</span>
                  </div>
                ))}
              </div>
            </div>
